      ...
      {today.length > 0 && (
        <section className="card card-pad">
          <div className="flex items-center justify-between">
            ...
            <span className="chip border-gray-200 bg-gray-50 text-gray-600">{today[0].durationMin} min · {today[0].distanceKm} km</span>
          </div>
          <div className="mt-2 flex items-baseline gap-2">
            <span className={cn('text-2xl font-bold', scoreTone(today[0].driveability).text)}>{today[0].driveability}</span>
            <span className="text-xs text-gray-500">driveability · {today[0].hazardsEncountered} hazards encountered</span>
          </div>
        </section>
      )}
        ...
        <section className="card divide-y divide-gray-100">
          {past.map((j) => (
            <div key={j.id} className="card-pad flex items-center gap-3">
              <div className="grid h-9 w-9 place-items-center rounded-lg bg-primary-50 text-primary-700" aria-hidden>
                <RouteIcon className="h-4.5 w-4.5" />
              </div>
              <div className="min-w-0 flex-1">
                <div className="truncate text-sm font-semibold text-ink">{j.from} → {j.to}</div>
                <div className="text-xs text-gray-500">{fmtDateShort(j.date)} · {j.durationMin} min · {j.distanceKm} km</div>
              </div>
              <div className="text-right">
                <div className={cn('text-sm font-bold', scoreTone(j.driveability).text)}>{j.driveability}</div>
                <div className="text-[10px] text-gray-400">driveability</div>
              </div>
            </div>
          ))}
        </section>
