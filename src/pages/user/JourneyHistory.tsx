      ...
      {today.length > 0 && (
        <section className="card card-pad">
          <div className="flex items-center justify-between">
            ...
            <span className="chip border-line bg-white/5 text-gray-400">{today[0].durationMin} min · {today[0].distanceKm} km</span>
          </div>
          <div className="mt-2 flex items-baseline gap-2">
            <span className={cn('text-2xl font-bold', scoreTone(today[0].driveability).text)}>{today[0].driveability}</span>
            <span className="text-xs text-gray-400">driveability · {today[0].hazardsEncountered} hazards encountered</span>
          </div>
        </section>
      )}
          ...
          {past.map((j) => (
            <div key={j.id} className="card-pad flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="text-xs text-gray-400">{fmtDateShort(j.date)} · {j.durationMin} min · {j.distanceKm} km</div>
              </div>
