      ...
      <section className="card card-pad flex flex-wrap items-center gap-3">
        ...
        <div className="flex flex-wrap gap-1.5" role="group" aria-label="Severity filter">
          {SEVS.map((s) => (
            <button
              key={s}
              className={cn('chip', sev === s ? 'border-ink bg-ink text-white' : 'border-gray-200 bg-white text-gray-600 hover:border-gray-300')}
              onClick={() => setSev(s)}
              aria-pressed={sev === s}
            >
              {s === 'all' ? 'All' : s}
        ...
        <div className="flex flex-wrap gap-1.5" role="group" aria-label="Status filter">
          {STATUSES.map((s) => (
            <button
              key={s}
              className={cn('chip', status === s ? 'border-ink bg-ink text-white' : 'border-gray-200 bg-white text-gray-600 hover:border-gray-300')}
              onClick={() => setStatus(s)}
        ...
        <div className="ml-auto flex flex-wrap gap-2 text-xs">
          {(Object.keys(layers) as (keyof typeof layers)[]).map((k) => (
            <label key={k} className="flex cursor-pointer items-center gap-1.5 font-medium text-gray-600">
      ...
      <section className="grid gap-4 xl:grid-cols-[1fr_340px]">
        ...
        <aside className="space-y-4" aria-label="Hazard details">
          ...
          <div className="card card-pad">
            <h4 className="mb-2 text-xs font-bold uppercase tracking-wider text-gray-500">Repair overlay</h4>
            {repairs.length === 0 ? (
